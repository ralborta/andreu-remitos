#!/usr/bin/env python3
"""
Setup Document AI — remitos MyESA / M&E (mismo schema que TSB).

Processor: 94d11150167e3e2a (MyESA Remitos M&E)
Fotos: /Users/ralborta/Andreu/M&E/*.jpeg

Uso:
  python3 scripts/gcp/setup-mye.py
"""
import base64
import json
import os
...
PROJECT = "kiev-prueba"
LOCATION = "us"
PROCESSOR = os.environ.get("MYE_PROCESSOR", "94d11150167e3e2a").strip()
BASE_MODEL = "pretrained-foundation-model-v1.5-pro-2025-06-20"
BUCKET = "gs://docai-kiev-beraldi/mye"
MYE_DIR = Path("/Users/ralborta/Andreu/M&E")
...
def list_mye_files():
    if not MYE_DIR.is_dir():
        raise FileNotFoundError(f"No existe {MYE_DIR}")
    files = sorted(
        p
        for p in MYE_DIR.iterdir()
        if p.suffix.lower() in (".png", ".jpg", ".jpeg", ".webp") and not p.name.startswith(".")
    )
    if len(files) < 2:
        raise RuntimeError(f"Se necesitan al menos 2 imágenes en {MYE_DIR}")
...
def schema_doc():
    ...
    if _SCHEMA:
        return _SCHEMA
    with open(ROOT / "scripts/gcp/schema-mye.json") as f:
...
def init_dataset():
    ...
    with urllib.request.urlopen(req, timeout=60) as r:
        json.load(r)
    schema_doc()
    print("Schema MyESA OK (16 campos)")
...
def upload_gcs(train, test):
    subprocess.run(
        ["gsutil", "mb", "-p", PROJECT, "-l", "US", "docai-kiev-beraldi"],
        stderr=subprocess.DEVNULL,
        check=False,
    )
    mapping = []
    for i, path in enumerate(train, 1):
        dst = f"{BUCKET}/train/mye-train-{i:02d}{path.suffix.lower()}"
    ...
    for i, path in enumerate(test, 1):
        dst = f"{BUCKET}/test/mye-test-{i:02d}{path.suffix.lower()}"
        subprocess.check_call(["gsutil", "-q", "cp", str(path), dst])
...
def process_image(gcs_uri, retries=3):
    ...
    json_uri = (
        gcs_uri.replace(f"{BUCKET}/train/", f"{BUCKET}/labeled-json/train/")
        .replace(f"{BUCKET}/test/", f"{BUCKET}/labeled-json/test/")
        .rsplit(".", 1)[0]
        + ".json"
    )
...
def create_foundation():
    ...
    url = (
        f"https://{LOCATION}-documentai.googleapis.com/v1/projects/{PROJECT}/"
        f"locations/{LOCATION}/processors/{PROCESSOR}/processorVersions:train"
    )
    req = urllib.request.Request(url, data=json.dumps(body).encode(), headers=headers(), method="POST")
    with urllib.request.urlopen(req, timeout=120) as r:
        op = json.loads(r.read())
    print("Creando mye-foundation-v1...")
    ...
    with urllib.request.urlopen(req, timeout=60) as r:
        json.load(r)
    print(f"mye-foundation-v1 listo: {pv}")
    return pv


def update_env(pv_id):
    keys = {
        "DOCUMENT_AI_CUSTOM_MYE_ID": PROCESSOR,
        "DOCUMENT_AI_MYE_VERSION": pv_id,
    }
...
def main():
    ...
    print("\n5) Creando mye-foundation-v1...")
    pv_id = create_foundation()
    update_env(pv_id)
    print(f"\nDONE. DOCUMENT_AI_MYE_VERSION={pv_id}")
