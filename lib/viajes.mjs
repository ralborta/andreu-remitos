...
export function normalizarAltaViaje(body = {}) {
  ...
  const tenant = ["tsb", "beraldi", "corina", "mye"].includes(tenantRaw) ? tenantRaw : null;
  ...
  return {
    ...
    tipo_carga: String(body.tipo_carga ?? body.tipoCarga ?? "").trim() || null,
    tipo_unidad: String(body.tipo_unidad ?? body.tipoUnidad ?? "").trim() || null,
    tenant,
    ...
    telefono_cliente: String(body.telefono_cliente ?? body.telefonoCliente ?? "").trim() || null,
