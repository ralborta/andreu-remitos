...
export function clasificarIntencionHeuristica(
  ...
  // Rendición solo aplica a choferes de Remitos; clientes no entran a ese intent.
  if (
    esChoferRemitos &&
...
Contexto:
- es_chofer_remitos: ${esChoferRemitos ? "sí" : "no"} (remitos + rendición)
- es_chofer_flota_viajes: ${esChoferFlotaViajes ? "sí" : "no"} (flota Gestión de Viajes → incidencias en ruta)
...
Intents:
- "incidencia": evento EN RUTA del chofer — pinchazo, accidente, control policial, desvío, parado, demora en viaje, problema mecánico, "incidencia". SOLO si es_chofer_remitos=sí O es_chofer_flota_viajes=sí. NO es queja de cliente.
- "rendicion": gastos menores del viaje — nafta, peajes, tickets, facturas, aceite, remolque, auxilio, arreglos, "rendición" (comprobante). SOLO si es_chofer_remitos=sí. Pinchazo SIN pedir rendir gasto → "incidencia".
...
- "remito": foto/corrección/confirmación de remito, km, patente, guía, "OK" de remito. SOLO si es_chofer_remitos=sí.
- "chat": saludo o pregunta general sin pedido concreto
- "desconocido": no se entiende

Reglas:
- Si es_chofer_remitos=no: NUNCA elijas "rendicion" ni "remito".
- Si es_chofer_remitos=no Y es_chofer_flota_viajes=no: NUNCA elijas "incidencia".
- Si (es_chofer_remitos=sí o es_chofer_flota_viajes=sí) y habla de evento en ruta / pinchazo / policía / accidente / estoy parado → "incidencia".
- Si es_chofer_remitos=sí y habla de gasto/comprobante/nafta/peaje/taller para rendir → "rendicion".
...
JSON:
{
  ...
  if (!esChoferRemitos && (intent === "rendicion" || intent === "remito")) {
    intent = "desconocido";
  }
  ...
  if (!esChoferRemitos && mensaje && /rendici[oó]n/i.test(mensaje)) {
    mensaje = emergency().mensaje;
  }
