...
export function clasificarIntencionHeuristica(texto, { esChoferRemitos = false } = {}) {
  ...
  // Viaje / flete — "necesito un viaje", "llevar carga", etc.
  if (
    /\b(viaje|flete|transporte|necesitamos|solicitamos|pedimos|necesito|retiro|camión|camion|tolva|toneladas?|\btn\b|llevar\s+carga|carga\s+a)\b/i.test(
      t,
    ) ||
    /\b(desde|de)\s+.+\s+(a|hacia|→)\s+/i.test(raw) ||
    /\ba\s+[A-ZÁÉÍÓÚÑ][\wáéíóúñ\s.-]{2,40}\s+para\s+llevar/i.test(raw)
  ) {
    return { intent: "viaje", confianza: 0.85, mensaje: null, fuente: "heuristica" };
  }
