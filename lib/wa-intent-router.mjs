...
export function clasificarIntencionHeuristica(
  ...
  // POP — retiro/carga en origen (antes de reclamo/rendición; "POP" ≠ queja)
  {
    const tn = t
      .normalize("NFD")
      .replace(/\p{M}/gu, "");
    if (
      esChoferOperativo &&
      /\b(pop|proof\s+of\s+pickup|registro\s+de\s+retiro|ya\s+cargue|cargue|retire|pickup|envi(ar|o)\s+(un\s+)?pop|quiero\s+(enviar\s+)?(un\s+)?pop)\b/.test(
        tn,
      ) &&
      !/\b(entregue|pod)\b/.test(tn)
    ) {
      return { intent: "pop", confianza: 0.95, mensaje: null, fuente: "heuristica" };
    }
  }
  ...
  if (
    ...
    !/\b(pinchazo|incidencia|accidente|polic[ií]a|pop)\b/i.test(t)
  ) {
  ...
  return {
    ...
    mensaje: esChoferOperativo
      ? "Hola 👋 ¿En qué te puedo ayudar?\n\n" +
        ...
        "• *POP* — registro de retiro en origen\n" +
        "• *POD* — constancia de entrega (receptor + foto)\n" +
...
/**
 * @returns {Promise<{
 *   intent: 'remito'|'viaje'|'reclamo'|'rendicion'|'incidencia'|'pop'|'pod'|'chat'|'desconocido',
 *   confianza: number,
 *   mensaje: string|null,
 *   fuente: string,
 * }>}
...
export async function clasificarIntencionWhatsApp({
  ...
  // Atajo duro: "quiero enviar mi remito" no depende de la IA (a veces devolvía chat/desconocido).
  if (esChoferIncidencia && pareceQuiereRemito(texto)) {
    return { intent: "remito", confianza: 1, mensaje: null, fuente: "heuristica" };
  }
...
- es_chofer_operativo: ${esChoferIncidencia ? "sí" : "no"} (cualquiera de los dos → remitos, rendición, incidencias, POD)
...
- "pop": registro / prueba de RETIRO en origen — "POP", "quiero enviar un POP", ya cargué, retiré, pickup, foto de carga/comprobante en origen. SOLO si es_chofer_operativo=sí. NO es POD ni reclamo.
- "pod": constancia / prueba de entrega — entrega hecha, receptor, firma/sello, foto de mercadería entregada, "POD", "entregué". SOLO si es_chofer_operativo=sí. Decidí por significado, NO por keywords exactas. NO es remito de carga ni gasto.
...
- "reclamo": QUEJA DE CLIENTE — demora de entrega, faltante, daño, extravío, producto equivocado. NUNCA si es chofer reportando algo en ruta. NUNCA si dice POP / registro de retiro.
...
Reglas:
- Si es_chofer_operativo=no: NUNCA elijas "rendicion" ni "remito" ni "incidencia" ni "pod" ni "pop".
- Si es_chofer_operativo=sí y habla de remito / foto de remito / guía / enviar remito → "remito".
- Si es_chofer_operativo=sí y habla de POP / retiro / cargué / pickup / comprobante de retiro → "pop".
- Si es_chofer_operativo=sí y habla de entrega hecha / receptor / constancia / POD / firma de entrega → "pod".
...
- Si es chofer operativo y saluda sin pedido → "chat" ofreciendo remito, POP, POD, incidencia, rendición o viaje.
...
JSON:
{
  ...
  if (
    !esChoferIncidencia &&
    (intent === "rendicion" || intent === "remito" || intent === "pod")
  ) {
    intent = "desconocido";
  }
