...
export function clasificarIntencionHeuristica(texto, { esChoferRemitos = false } = {}) {
  ...
  // Rendición solo aplica a choferes registrados; clientes no entran a ese intent.
  if (
    esChoferRemitos &&
  ...
  // Incidencia en ruta — solo choferes (antes de reclamo: "demora" del chofer ≠ queja cliente)
  if (
    esChoferRemitos &&
...
Contexto:
- es_chofer_remitos: ${esChoferRemitos ? "sí" : "no"} (si sí: remitos + incidencias + rendición; si no: es CLIENTE)
...
Intents:
- "incidencia": evento EN RUTA del chofer — pinchazo, accidente, control policial, desvío, parado, demora en viaje, problema mecánico, "incidencia". SOLO si es_chofer_remitos=sí. NO es queja de cliente.
...
Reglas:
- Si es_chofer_remitos=no: NUNCA elijas "rendicion", "remito" ni "incidencia".
- Si es_chofer_remitos=sí y habla de evento en ruta / pinchazo / policía / accidente / estoy parado → "incidencia".
...
JSON:
{
  ...
  if (!esChoferRemitos && (intent === "rendicion" || intent === "remito" || intent === "incidencia")) {
    intent = intent === "incidencia" ? "reclamo" : "desconocido";
  }
