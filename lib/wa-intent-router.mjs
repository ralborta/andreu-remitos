...
export function clasificarIntencionHeuristica(
  ...
  // POD / constancia de entrega
  if (
    esChoferOperativo &&
    /\b(pod|constancia|entregu[eé]|entregue|entregado|prueba\s+de\s+entrega|recib[ií]\s*conforme|firma\s+de\s+entrega|descargu[eé])\b/i.test(
      t,
    ) &&
    !/\b(remito|gu[ií]a|nafta|peaje|rendici[oó]n|gasto)\b/i.test(t)
  ) {
    return { intent: "pod", confianza: 0.85, mensaje: null, fuente: "heuristica" };
  }
...
/**
 * @returns {Promise<{
 *   intent: 'remito'|'viaje'|'reclamo'|'rendicion'|'incidencia'|'pod'|'chat'|'desconocido',
 *   confianza: number,
 *   mensaje: string|null,
 *   fuente: string,
 * }>}
...
- "pod": constancia / prueba de entrega — "entregué", "POD", a quién entregó, foto de entrega/firma/sello. SOLO si es_chofer_operativo=sí. NO es remito de carga ni gasto.
...
Reglas:
- Si es_chofer_operativo=no: NUNCA elijas "rendicion" ni "remito" ni "incidencia" ni "pod".
...
- Si es chofer operativo y saluda sin pedido → "chat" ofreciendo remito, POD, incidencia, rendición o viaje.
