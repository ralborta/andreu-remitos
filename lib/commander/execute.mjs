...
export async function executeCommanderDecision(decision, ctx, deps) {
  ...
  if (
    ...
    if (key === "pop_solo_choferes") {
      const m = mensajePopSoloChoferes();
      await notificarChofer(ev.from, m, { log, tenant: null }).catch(() => {});
      return reply({
        message: m,
        flow: "pop_solo_choferes",
        received: true,
        commander: decision.decisionId,
      });
    }
    ...
    if (key === "clarify" || key === "remitos_solo_choferes") {
      ...
      // notificarChofer ya persiste el outbound; no append duplicado (evita saludo x2 en Contactos).
      await notificarChofer(ev.from, msg, { log, tenant: null }).catch(() => {});
      return reply({
        message: msg,
  ...
  switch (key) {
    ...
    case "pop_force":
    case "pop_force_media": {
      const { imageBuffer, mime } = await mediaBuffers();
      const out = await tryProcesarPop(ev, { texto, log, forzar: true, imageBuffer, mime });
      if (out) return reply({ ...out, received: true, commander: decision.decisionId });
      const msg =
        `Perfecto, vamos con el *registro de retiro (POP)*.\n\n` +
        `Mandame *fotos claras* de la carga / comprobante de retiro.`;
      await notificarChofer(ev.from, msg, { log, tenant: null }).catch(() => {});
      return reply({ message: msg, flow: "pop_fallback", received: true, commander: decision.decisionId });
    }
