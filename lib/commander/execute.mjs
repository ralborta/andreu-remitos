...
export async function executeCommanderDecision(decision, ctx, deps) {
  ...
  async function maybeAutoResumeAfterEphemeral(out) {
    if (!isCommanderV11InterruptEnabledForSubject(ev.from)) return out;
    if (decision.processType !== "ephemeral_qa") return out;
    const top = interruptApi.peekStackTop(ev.from);
    if (!top || top.resumeMode !== "auto_on_child_idle") return out;
    try {
      const popped = interruptApi.popResume({
        subjectId: ev.from,
        decisionId: decision.decisionId,
      });
      if (popped.ok && popped.resumeHint) {
        await notificarChofer(ev.from, popped.resumeHint, { log, tenant: null }).catch(() => {});
        return {
          ...out,
          resume_hint: popped.resumeHint,
          resumed_process: popped.parent?.processId,
          flow: out?.flow || "interrupt_ephemeral_then_resume",
        };
      }
    } catch (err) {
      log?.warn?.({ err: err?.message }, "v1.1 auto-resume after ephemeral failed");
    }
    return out;
  }
  ...
  if (
    ...
    if (key === "clarify" || key === "remitos_solo_choferes") {
      ...
      const withResume = await maybeAutoResumeAfterEphemeral(out);
      return respuestaWebhook(withResume);
    }
  }
