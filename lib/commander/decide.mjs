...
async function decideV11(input) {
  ...
  if (texto && !hasMedia && isExplicitCancelText(texto)) {
    ...
    if (active) {
      ...
      if (after) {
        const popped = resumeTop({ subjectId, completeChild: false, reason: "cancel_active_then_resume_top" });
        if (popped.ok) {
          const decision = finalizeDecision({
  ...
  // Resume manual → resume_parent (o resume_until si resumeTargetProcessId explícito)
  if (texto && !hasMedia && isExplicitResumeText(texto) && peekStackTop(subjectId)) {
    const targetId = input.resumeTargetProcessId || null;
    const popped = targetId
      ? resumeUntil({
          subjectId,
          processId: targetId,
          completeChild: true,
          reason: "manual_resume_until",
        })
      : resumeParent({ subjectId, completeChild: true });
    if (popped.ok) {
      const decision = finalizeDecision({
        intent: "continue_process",
        confidence: 1,
        ...
        suggestedReply: popped.resumeHint,
        interrupt: {
          op: popped.mode || (targetId ? "resume_until" : "resume_parent"),
          mode: popped.mode || (targetId ? "resume_until" : "resume_parent"),
          frameId: popped.frame?.frameId ?? null,
          parentProcessId: popped.parent.processId,
          depth: popped.frame?.depth ?? null,
          pops: popped.pops ?? null,
        ...
        trace: {
          ...
          notes: ["manual_resume", popped.mode, targetId ? "with_target" : "parent"],
        },
      });
      logCommanderTrace(log, decision, { subjectId, v11: true });
      return decision;
    ...
    log?.warn?.(
      { err: popped.reason, mode: popped.mode, targetId },
      "v1.1.1 resume failed → V1",
    );
...
export async function decide(input) {
  ...
  // Fuera de allowlist (o flag OFF) → exactamente V1
  if (!isCommanderV11InterruptEnabledForSubject(input.message?.subjectId)) {
    return decideV1(input);
  }
