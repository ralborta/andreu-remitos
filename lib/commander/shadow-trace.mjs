 ...
 * @param {object|null} [conversation] — opcional; si falta se lee del store
 */
export async function detectActiveProcesses(subjectId, conversation = null) {
  if (!subjectId) return [];
  const out = [];
  try {
  ...
  try {
    const pop = evidenceStore.getEvidenceDialogByTelefono(subjectId, { type: "POP" });
    if (pop) out.push({ processId: pop.id ?? null, processType: "pop_caso", agentId: "pod" });
  } catch { /* ignore */ }
  try {
  ...
  try {
    const conv =
      conversation ??
      (await convStore.getConversacion(subjectId).catch(() => null));
    const remitoId = resolveRemitoRevisionId(conv);
    if (remitoId) {
      out.push({ processId: remitoId, processType: "remito_revision", agentId: "remitos" });
    }
  } catch { /* ignore */ }
  return out;
}
...
export function inferLegacyDecisionFromPayload(payload = {}) {
  ...
  const map = [
    ...
    { re: /^pop/, intent: "pop", agentId: "pod", action: "run_agent" },
    { re: /^pod/, intent: "pod", agentId: "pod", action: "run_agent" },
    { re: /^rendicion/, intent: "rendicion", agentId: "rendicion", action: "run_agent" },
    { re: /^destino/, intent: "destino", agentId: "destinos", action: "run_agent" },
    { re: /^viaje/, intent: "viaje", agentId: "viajes", action: "run_agent" },
    { re: /remito_pedir_foto|remito_revision/, intent: "remito", agentId: "remitos", action: "run_agent" },
...
export function noteLegacyResponse(payload, log) {
  ...
  const trace = {
    ...
    shadow: true,
    v1Active: Boolean(store.v1Active),
    decideMs: store.decideMs ?? null,
    ...
    agentId: d.agentId,
    action: d.action,
    processBinding: Boolean(d.processBinding),
