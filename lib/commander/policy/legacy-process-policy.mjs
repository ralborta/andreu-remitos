 ...
 *
 * v1.0.1 — parity semántico:
 * - proceso activo → intent de dominio + action=run_agent + processBinding
 * - remito_revision como sticky (después de early binds que AS-IS tienen prioridad)
 */
...
import * as evidenceStore from "../../../backend/src/db/evidence-store.mjs";
...
import { parecePopHeuristica } from "../../pop-wa.mjs";
import { pareceQuiereRemito } from "../../wa-intent-router.mjs";
import { finalizeDecision } from "../trace/log-trace.mjs";

/**
 * Binding de proceso activo: dominio + run_agent (compatible executor legacy).
 * @param {object} partial
 */
function bindProcess(partial) {
  return {
    handled: true,
    decision: finalizeDecision({
      ...partial,
      processBinding: true,
      action: partial.action ?? "run_agent",
      intentSource: partial.intentSource ?? "policy",
      forceAgent: partial.forceAgent !== false,
...
/** Misma regla que flujoRemitoAbierto / resolverRemitoCorreccion en webhooks (sin tocar Remitos). */
export function resolveRemitoRevisionId(conversation) {
  if (!conversation) return null;
  if (conversation.remito_en_revision_id) return conversation.remito_en_revision_id;
  if (
    conversation.ultimo_remito_id &&
    conversation.ultimo_remito_id !== conversation.remito_cerrado_id
  ) {
    return conversation.ultimo_remito_id;
  }
  return null;
}
...
export async function evaluateLegacyProcessPolicy(input) {
  const { message, actor, log, conversation = null } = input;
  ...
  const quierePopHard = Boolean(texto) && parecePopHeuristica(texto);

  // 0. POP explícito — antes de stickies (reclamo/destinos no deben robar "quiero mandar un POP")
  if (actor.isChoferOperativo && quierePopHard) {
    const pendingPopEarly = evidenceStore.getEvidenceDialogByTelefono(subjectId, { type: "POP" });
    return bindProcess({
      intent: "pop",
      confidence: 1,
      agentId: "pod",
      processType: "pop_caso",
      processId: pendingPopEarly?.id ?? null,
      executorHints: { executorKey: "pop_force", legacyFlow: "pop_heuristica" },
      notes: ["sticky:pop_heuristica"],
    });
  }

  // 1. Reclamo pendiente
  const pendingReclamo = await reclamosStore.getReclamoPendientePorTelefono(subjectId);
  if (pendingReclamo && (texto || esFoto) && !quierePopHard) {
    return bindProcess({
      intent: "reclamo",
      confidence: 1,
      agentId: "reclamos",
  ...
  // 4. POP pendiente (antes de POD)
  const pendingPop = evidenceStore.getEvidenceDialogByTelefono(subjectId, { type: "POP" });
  if (pendingPop && (texto || esFoto)) {
    return bindProcess({
      intent: "pop",
      confidence: 1,
      agentId: "pod",
      processType: "pop_caso",
      processId: pendingPop.id ?? null,
      executorHints: { executorKey: "pop_force", legacyFlow: "pop_pending" },
      notes: ["sticky:pop_pending"],
    });
  }
  ...
  if (actor.isChoferOperativo && !pendingPod && !pendingPop && texto && !pareceQuiereRemito(texto)) {
  ...
  // No robar si el chofer pide POP explícitamente (ya cubierto arriba; defensa extra).
  const pendingCliente = await destinosStore.getDestinoPendientePorTelefono(subjectId);
  if (pendingCliente && !quierePopHard) {
  ...
  if (pendingChofer && !esFoto && !message.hasMedia && !quierePopHard) {
    return bindProcess({
      intent: "destino",
      confidence: 1,
      agentId: "destinos",
  ...
  // 10b. Remito en revisión — binding sticky (después de early binds AS-IS: incidencia/rendición/destinos/viaje)
  const remitoRevisionId = resolveRemitoRevisionId(conversation);
  if (remitoRevisionId && texto && !esFoto && mediaKind !== "audio" && mediaKind !== "voice_note") {
    return bindProcess({
      intent: "remito",
      confidence: 1,
      agentId: "remitos",
      processType: "remito_revision",
      processId: remitoRevisionId,
      executorHints: { executorKey: "remitos_texto", legacyFlow: "remito_revision" },
      notes: ["sticky:remito_revision"],
    });
  }
  ...
  if (mediaKind === "audio" || mediaKind === "voice_note") {
    return dec({
      ...
      agentId: "remitos",
      action: "run_agent",
      processBinding: Boolean(remitoRevisionId),
      processType: remitoRevisionId ? "remito_revision" : null,
      processId: remitoRevisionId,
