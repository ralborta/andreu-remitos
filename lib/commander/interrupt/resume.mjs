...
export function applyResumeFromExecutionResult({
  ...
  // failed: trazar, preservar binding active, nunca pop destructivo
  if (result.status === "failed") {
    markActiveFailedNoPop({ subjectId, decisionId });
    return { ok: false, reason: "failed_no_pop", result };
  }

  if (result.status !== "completed" || result.resumePolicy !== "auto") {
...
/**
 * Marca fallo de ejecución del active sin pop y sin romper el binding.
 * status orquestación permanece `active`; el fallo vive en lastExecutionStatus.
 */
export function markActiveFailedNoPop({ subjectId, decisionId = null } = {}) {
  const orch = getOrchestration(subjectId);
  const active =
    getActiveProcess(subjectId) ||
    (orch.activeProcessId ? getProcess(orch.activeProcessId) : null);
  if (!active) return { ok: false, reason: "no_active" };

  const updated = upsertProcess({
    ...active,
    status: "active",
    lastExecutionStatus: "failed",
  });

  // Asegurar puntero orch (por si venía de residual failed)
  if (orch.activeProcessId !== updated.processId) {
    orch.activeProcessId = updated.processId;
  ...
  traceInterruptTransition({
    ...
    subjectId,
    parentProcessId: updated.processId,
    processType: updated.processType,
    reason: "failed_no_destructive_pop",
    decisionId,
    extra: { lastExecutionStatus: "failed", bindingKept: "active" },
  });
