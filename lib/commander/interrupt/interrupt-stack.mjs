...
export function ensureActiveFromDomain({
  ...
  if (orch.activeProcessId) {
    const cur = getProcess(orch.activeProcessId);
    if (cur && cur.status === "active") return { process: cur, orchestration: orch, created: false };
  }
...
/**
 * Si el active está terminal y el tope del stack pide auto_on_child_complete → pop.
 * Usado por tests y por executor cuando el hijo cierra.
 */
export function tryAutoResumeAfterChildComplete({ subjectId, decisionId = null } = {}) {
  ...
  if (child && child.status === "active") {
    // Caller marks terminal; if still active, treat as completed for auto path
    upsertProcess({ ...child, status: "completed" });
  }
  return popResume({ subjectId, decisionId, completeChild: false });
}
