...
async function transcribirConOpenAI(buffer, opts) {
  ...
  form.append(
    "prompt",
    "Corrección de remito de transporte: peso en kg, km, semi, acoplado, chasis, patente, destino, origen, horarios, guía.",
  );
  ...
  // Alucinaciones típicas de Whisper en audios muy cortos o silencio
  if (/^(subtitles?|thanks for watching|gracias por ver|\.{2,}|…+)$/i.test(text) || /amara\.org/i.test(text)) {
    text = "";
  }

  if (!text) throw new Error("OpenAI no devolvió texto útil");
  opts.log?.info?.({ engine: "openai", model, len: text.length }, "Audio transcrito");
  return text;
}
