...
/** Puntúa señales TSB / MyESA / Beraldi en texto OCR. Mayor = más probable. */
export function puntuarTenant(texto) {
  const u = String(texto ?? "").toUpperCase();
  const scores = { tsb: 0, beraldi: 0, corina: 0, mye: 0, señales: { tsb: [], beraldi: [], mye: [] } };
  ...
  // —— MyESA / M&E (muy específico) ——
  if (/\bM\s*&\s*E\b/.test(u) || /\bM\s*&\s*E\s*S\.?\s*A\.?\b/.test(u)) add("mye", 14, "marca M&E");
  if (/\bMYESA\b/.test(u) || /\bMYSSA\b/.test(u) || /MYESA\.COM/.test(u)) add("mye", 12, "marca MyESA");
  if (/\bME\s*S\.?\s*A\.?\b/.test(u) && /BASE\s+A[NÑ]ELO|REMITO|A[NÑ]ELO/.test(u)) add("mye", 10, "ME S.A.");
  if (/ADMINISTRACION@MYESA/.test(u)) add("mye", 10, "email myesa");
  if (/DESTINO\s*\/?\s*POZO/.test(u)) add("mye", 8, "DESTINO/POZO");
  if (/LUGAR\s+DE\s+CARGA/.test(u) && /PESO\s+NETO/.test(u)) add("mye", 6, "LUGAR DE CARGA + Peso Neto");
  if (/BASE\s+A[NÑ]ELO/.test(u)) add("mye", 8, "BASE AÑELO");
  if (/PRODUCTO\s+TRANSPORTADO/.test(u) && /ACOPLADO/.test(u)) add("mye", 3, "PRODUCTO + ACOPLADO");
  if (/LLEGA\s+A\s+LUGAR\s+DE\s+CARGA/.test(u) && /SALE\s+DE\s+LUGAR\s+DE\s+DESCARGA/.test(u)) {
    add("mye", 4, "horarios MyESA");
  }
  ...
  if (/REMITO\s*N[°º]?\s*\d{5}/.test(u) && !/GUIA\s+DE\s+TRANSPORTE/.test(u) && scores.mye < 6) {
    add("beraldi", 3, "formato REMITO N°");
  }
  ...
  if (/\bCHOFER\b/.test(u) && !/\bCONDUCTOR\s*:/.test(u) && scores.mye < 6) add("beraldi", 3, "etiqueta CHOFER");
  ...
  if (/GUIA\s+DE\s+TRANSPORTE/.test(u) && scores.beraldi < 4 && scores.mye < 4) add("tsb", 1, "Guía de transporte (débil)");
  if (/COMPA[NÑ]IA\s+DE\s+TRANSPORTES/.test(u) && !/\bBERALDI\b/.test(u) && scores.beraldi < 4 && scores.mye < 4) {
...
export function detectarTenantConConfianza(texto) {
  ...
  if (scores.mye >= 6) {
    return { tenant: "mye", confianza: Math.min(1, scores.mye / 20), margen: scores.mye, scores };
  }

  const tsb = scores.tsb;
  const beraldi = scores.beraldi;
  const mye = scores.mye;
  const total = tsb + beraldi + mye;
  ...
  const margen = bestScore - second;
  const confianza = total > 0 ? margen / total : 0;

  return { tenant: best, confianza, margen, scores };
}
...
- "mye": Remitos MyESA / M&E. Suele decir myesa.com.ar, DESTINO/POZO, LUGAR DE CARGA, CAMION, ACOPLADO, PRODUCTO TRANSPORTADO, Peso Neto.
...
Respondé SOLO JSON:
{"tenant":"tsb"|"beraldi"|"corina"|"mye"|null,"confianza":0.0-1.0,"motivo":"breve"}`;
  ...
  try {
    ...
    if (!["tsb", "beraldi", "corina", "mye"].includes(t)) return null;
