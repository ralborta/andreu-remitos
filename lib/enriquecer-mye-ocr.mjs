...
import { extraerMye, normalizarPeso } from "./extract-cold.mjs";
...
function valorLugar(valor) {
  const s = limpiar(valor);
  if (!s || esEtiquetaCampoRemito(s)) return null;
  const cortado = limpiar(
    s.replace(
      /\s*(DESTINO\s*\/?\s*POZO|LUGAR\s+DE\s+CARGA|CHOFER|EQUIPO|CAMION|ACOPLADO|PRODUCTO|CANTIDAD).*$/i,
      "",
    ),
  );
  if (!cortado || esEtiquetaCampoRemito(cortado)) return null;
  return cortado;
}

function mejorLugar(actual, fallback) {
  const a = valorLugar(actual);
  const b = valorLugar(fallback);
  if (!a) return b;
  if (!b) return a;
  if (esEtiquetaCampoRemito(actual) && b) return b;
  return a;
}
...
export function enriquecerMyeDesdeTexto(datos, textoOcr = "") {
  ...
  const d = { ...datos };
  d.procedencia = valorLugar(d.procedencia);
  d.destino = valorLugar(d.destino);
  if (!textoOcr?.trim()) return d;

  const t = textoOcr;
  const horarios = parsearHorarios(textoOcr);
  const frio = extraerMye(textoOcr);
  d.procedencia = mejorLugar(d.procedencia, frio.procedencia);
  d.destino = mejorLugar(d.destino, frio.destino);
