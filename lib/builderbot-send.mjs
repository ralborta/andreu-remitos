...
/** Indicador "escribiendo…" en WhatsApp del chofer (solo Baileys). */
export async function sendWhatsAppTyping(number) {
  if (!BAILEYS_BOT_URL) return { ok: true, skipped: true };
  const phone = String(number).replace(/\D/g, "");
  if (phone.length < 9) return { ok: false, error: "number inválido" };
...
async function sendViaBaileysBot({ number, message, mediaUrl }) {
  const phone = String(number).replace(/\D/g, "");
  if (phone.length < 9) {
    throw new Error("number inválido para Baileys");
  }
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      lastErr = err;
      // Reintento: contactos nuevos a veces fallan al primer envío (sesión Signal).
      if (attempt < 2) await sleep(700);
    }
  }
  throw lastErr ?? new Error("Bot Baileys: envío fallido");
}
