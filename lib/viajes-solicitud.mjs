...
export function pareceSolicitudViaje(texto) {
  ...
  const tieneRuta =
    ...
    /\bviaje\s+a\s+[\wáéíóúñ\s.-]{2,40}/i.test(raw) ||
    /\ba\s+[\wáéíóúñ\s.-]{2,40}\s+para\s+llevar/i.test(raw);

  const tieneCarga = /\b\d{1,3}\s*(t|tn|ton|toneladas?)\b/i.test(raw);
  const keywords =
    /\b(necesitamos|solicitamos|pedimos|necesito|transporte|viaje|retiro|entrega|carga|movimiento|flete|llevar\s+carga)\b/i.test(
      t,
    );
  const mencionaCampos =
    /\b(origen|destino|retiro|toneladas?|\btn\b|tipo\s+de\s+carga|fecha|hora)\b/i.test(t);

  // Pedido explícito de viaje/flete (aunque falten datos)
  if (/\b(necesito|necesitamos|solicitamos|pedimos)\b.*\b(viaje|flete|transporte)\b/i.test(t)) {
    return true;
  }
  if (/\b(viaje|flete)\b/i.test(t) && (tieneRuta || tieneCarga || /llevar|carga|origen|destino/i.test(t))) {
    return true;
  }
  // Respuesta con datos de viaje (origen/destino/tn…) sin decir “viaje”
  if (mencionaCampos && (tieneCarga || tieneRuta || /\b(origen|destino)\b/i.test(t))) {
    return true;
  }

  return (tieneRuta && (tieneCarga || keywords)) || (keywords && mencionaCampos);
}
