...
/**
 * Visión: lee formulario POD / evidencia de entrega (100% IA).
 */
export async function leerPodDesdeImagen({ imageBuffer, mime, texto, log } = {}) {
  if (!iaHabilitada() || !imageBuffer?.length) return null;

  const imageBase64 = Buffer.from(imageBuffer).toString("base64");
  const system =
    "Sos el agente POD (Proof of Delivery) de TransitOne / SOL. " +
    "Leés fotos de constancias/formularios de entrega y/o mercadería entregada. " +
    "Respondé SOLO JSON válido. No inventes datos que no se vean.";

  const userContent = [
    {
      type: "text",
      text: `Analizá esta foto de constancia de entrega (POD) o evidencia de entrega.

Extraé solo lo que se ve:
- receptor_nombre: quien recibió (destinatario / firma)
- pedido_ref: nº de pedido, tracking, PE-…, ECO-…, guía
- destino: dirección o ciudad
- chofer_documento: repartidor si figura en el papel
- resumen: 1 línea de lo que ves
- mensaje: texto WhatsApp corto confirmando lo leído y avisando que queda pendiente de mesa (rioplatense). Incluí receptor y ref si los hay.
