...
import { corregirPatenteMercosur } from "./patente-ocr.mjs";
import {
  mapearLugaresCorina,
...
export function normalizarPatente(val) {
  ...
  const corregida = corregirPatenteMercosur(s);
  return corregida || s;
}
...
export function normalizarDatosRemito(datos, tenant) {
  ...
  if (tenant === "tsb") {
    ...
    if ((!d.semi || String(d.semi).trim() === "") && d.tractor) {
      ...
      if (semi) {
        d.semi = semi;
        d.patente_acoplado = semi;
      }
    }
    if (d.conductor) d.conductor = normalizarChoferCorina(d.conductor) ?? d.conductor;
    if (d.chofer) d.chofer = normalizarChoferCorina(d.chofer) ?? d.chofer;
  }
  ...
  if (tenant === "corina") {
    Object.assign(cleaned, mapearLugaresCorina(cleaned, ""));
    if (!cleaned.cliente_marca) {
      const marca = inferirMarcaClienteCorina(cleaned, "");
      if (marca) cleaned.cliente_marca = marca;
    }
  }
