...
/**
 * Ajusta fechas cuando carga es de noche y descarga al día siguiente (misma fecha en OCR).
...
export function prepararHorariosRemito(horarios, fechaRemito = null) {
  ...
  const minSalida = aMinutos(slots.carga_salida.fecha, slots.carga_salida.hora);
  const minLlegada = aMinutos(slots.descarga_llegada.fecha, slots.descarga_llegada.hora);

  if (
    minSalida != null &&
    minLlegada != null &&
    minLlegada < minSalida &&
    slots.carga_salida.hora &&
    slots.descarga_llegada.hora
  ) {
    viajeNocturno = true;
    const fechaDescarga = sumarDias(base, 1);
    if (fechaDescarga) {
      for (const campo of ORDEN_CAMPOS) {
        if (campo.startsWith("descarga")) slots[campo].fecha = fechaDescarga;
      }
      advertencias.push(
        `Viaje nocturno: descarga (${slots.descarga_llegada.hora}–${slots.descarga_fin.hora ?? "?"}) registrada el ${fechaDescarga} (día siguiente a la carga).`,
      );
    }
  }
