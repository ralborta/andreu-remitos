...
import {
  buildPlanillaTmsQmResult,
} from "./planilla-tms-qm.mjs";
...
export { PLANILLA_TMS_QM_COLUMNS, remitoAFilasTmsQm } from "./planilla-tms-qm.mjs";
...
export async function buildPlanillaTsb(opts = {}) {
  ...
  const [remitos, choferes, localidades, unidades] = await Promise.all([
    remitoStore.listRemitos({ tenant, limit: Math.max(limit, 5000) }),
  ...
  if (formato === "qm") {
    return buildPlanillaTmsQmResult({
      ...
      tmsQmHooks: {
        findChofer: (remito, d) =>
          findChofer(ctx.choferes, remito.telefono_chofer, d.conductor ?? d.chofer),
        formatoChofer: formatoChoferTsb,
      },
