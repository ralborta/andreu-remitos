...
/**
 * Detecta hoja de ruta por texto OCR del papel (sin depender de caption/sticky).
 * Evita que caiga en la cola de remitos.
 */
export function pareceDocumentoHojaRuta(ocrTexto) {
  const t = String(ocrTexto ?? "").toLowerCase();
  if (!t.trim()) return false;
  if (/hoja\s*de\s*ruta/.test(t)) return true;
  // Formulario Andreu típico: "N° Viaje" + datos de chofer/patente
  const tieneNroViaje = /n[°ºo.]?\s*viaje|nro\.?\s*viaje|numero\s*de\s*viaje/.test(t);
  const tieneContexto =
    /\b(chofer|patente|interno|anticipo|semi|salida|llegada|andreu)\b/.test(t);
  return tieneNroViaje && tieneContexto;
}
...
export function mensajeConfirmacionHojaRuta(row) {
  ...
  return (
    ...
    `Ahora mandá las *fotos de los tickets* (peaje, nafta, etc.) *de a una*.\n` +
    `Cuando no tengas más, escribí *listo*.`
  );
}
