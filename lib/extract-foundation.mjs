...
import { enriquecerMyeDesdeTexto } from "./enriquecer-mye-ocr.mjs";
...
export function extraerBeraldiFoundation(campos, textoOcr = "") {
  ...
  const horariosBlock = {
    ...
    validacion: validarOrdenHorarios(horariosRaw, { fechaRemito }),
  };
  if (horariosBlock.validacion.horarios) {
    horariosBlock.horarios = horariosBlock.validacion.horarios;
  }

  const destino = campos.destino ?? null;
  const datos = enriquecerBeraldiDesdeTexto(
    {
      tenant: "beraldi",
      nro_remito: normalizarNroRemitoGuia(campos.nro_remito, { tenant: "beraldi" }),
      fecha_remito: fechaRemito,
      ot: campos.ot ? String(campos.ot).replace(/\D/g, "") || null : null,
      ...
      destino,
      km_inicial: campos.km_inicial ?? null,
      km_final: campos.km_final ?? null,
...
export function extraerTSBFoundation(campos, textoOcr = "") {
  ...
  const horariosBlock = {
    ...
    horarios: horariosRaw,
    validacion: validarOrdenHorarios(horariosRaw, { fechaRemito: fechaGuia }),
  };
  if (horariosBlock.validacion.horarios) {
    horariosBlock.horarios = horariosBlock.validacion.horarios;
  }
...
/** @param {Record<string, string>} campos @param {string} [textoOcr] */
export function extraerMyeFoundation(campos, textoOcr = "") {
  const fechaGuia = normalizarFecha(campos.fecha);
  ...
  const datos = enriquecerMyeDesdeTexto(
    {
      tenant: "mye",
      nro_guia: normalizarNroRemitoGuia(campos.nro_guia),
      fecha_guia: fechaGuia,
