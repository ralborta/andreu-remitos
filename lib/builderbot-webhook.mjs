...
export function normalizeBuilderBotPayload(body) {
  ...
  else if (eventName.includes("voice") || eventName.includes("audio")) event = "audio";
  ...
  const attachmentMime = extractMediaMime(attachment);
  if (mediaUrl) {
    const mime =
      attachmentMime ??
      (eventName.includes("audio") ? "audio/ogg" : "image/jpeg");
    media = {
      url: mediaUrl,
      mime_type: mime,
    };
    if (/^audio\//i.test(mime) || eventName.includes("audio")) {
      event = "audio";
    } else {
      event = "media";
    }
...
export function mensajeSaludo(_tenant) {
