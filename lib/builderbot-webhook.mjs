...
export function resolveTenant(telefono, explicitTenant) {
  ...
  if (t === "tsb" || t === "beraldi" || t === "corina" || t === "mye") return t;
  ...
  if (def === "tsb" || def === "beraldi" || def === "corina" || def === "mye") return def;
  return null;
}
...
export function mensajeWhatsApp(resultado) {
  ...
  const empresa =
    !tenantRaw || tenantRaw === "tsb" || tenantRaw === "mye"
      ? BRAND.remitoTenantLabel
      : String(tenantRaw).toUpperCase();
  ...
  if (tenantRaw === "corina") {
    const cliente =
      d.cliente_marca ||
      (typeof d.cliente === "string" && !/cervecer/i.test(d.cliente) ? d.cliente : null) ||
      d.cliente;
    const bloquesCorina = [
      ...
      linea("Pallets CHEP", d.pallets_chep != null ? String(d.pallets_chep) : d.total_bultos != null ? String(d.total_bultos) : null),
      linea("Tractor", d.tractor ?? d.patente_chasis ?? d.patente),
      linea("Semi", d.semi ?? d.patente_acoplado),
      linea("TR", d.tr ?? d.nro_tr),
      linea("Entrega", d.entrega ?? d.nro_entrega),
      linea("Pedido", d.pedido ?? d.nro_pedido),
    ].filter(Boolean);
    ...
    switch (resultado.estado) {
      ...
      case "confirmado":
        return `${bloquesCorina.join("\n")}\n\n✅ Todo validado.\n¡Buen viaje!`;
      default:
        return `📷 Recibí tu remito. Nro: ${guia}. Procesando…`;
    }
  }
  ...
  const destino = d.destino_nombre ?? d.destino_locacion ?? d.destino;
  const peso = d.peso_kg ?? d.peso;
  const ot = d.ot != null && String(d.ot).trim() !== "" ? String(d.ot).trim() : null;
  ...
  switch (resultado.estado) {
    ...
    case "bloqueado":
      return `${bloques.join("\n")}\n\n${pieBloqueado}`;
    default:
      return `📷 Recibí tu remito ${empresa}. Guía: ${guia}. Procesando…`;
  }
}

export function mensajeSaludo(tenant, nombreChofer) {
  const primerNombre = nombreChofer?.trim().split(/\s+/)[0];
  const hola = primerNombre ? `Hola ${primerNombre} 👋` : "Hola 👋";
  if (tenant === "corina") {
    return (
      `${hola} Soy el asistente de remitos ${BRAND.name}.\n\n` +
      `Antes de la foto, ¿qué cliente es el viaje?\n\n` +
      `Respondé:\n` +
      `• *Cervecería* (CMQ / Quilmes)\n` +
      `• *Eco* (Eco de los Andes)`
    );
  }
  return `${hola} Soy el asistente de remitos ${BRAND.name}.\n\nPodés:\n• Mandar una *foto clara* del remito\n• *Escribir* correcciones si algo no coincide\n• Decir o escribir *OK* cuando esté todo bien`;
}

/** Tras elegir Cervecería / Eco — pide la foto. */
export function mensajeCorinaListoParaFoto(marca) {
  return (
    `Perfecto — cliente *${marca}*.\n\n` +
    `Ahora mandame una *foto clara* del remito (donde se vean las paletas CHEP).`
  );
}

/** Bloquea foto Corina sin cliente elegido. */
export function mensajeCorinaFaltaCliente() {
  return (
    `Antes de procesar la foto, indicá el cliente del viaje:\n\n` +
    `• *Cervecería* (CMQ / Quilmes)\n` +
    `• *Eco* (Eco de los Andes)`
  );
}
...
/**
 * @param {string} texto
 * @returns {"Cervecería"|"Eco de los Andes"|null}
 */
export function parseClienteMarcaCorina(texto) {
  const t = String(texto ?? "")
    ...
    .trim();
  if (!t) return null;
  if (/^(eco|eco de los andes|ecoandes|andes)$/.test(t) || /\beco\b/.test(t)) {
    return "Eco de los Andes";
  }
  if (
    /^(cerveceria|quilmes|cmq|malteria)$/.test(t) ||
    /cervecer/.test(t) ||
    /\bquilmes\b/.test(t) ||
    /\bcmq\b/.test(t)
  ) {
...
export function mensajeAudioSoloConfirmacion() {
  return (
    ...
    "Para *corregir* algo, podés dictarlo por audio o escribirlo (ej: Peso 29762)."
  );
}

export function mensajeAudioSinCorreccion(transcripcion) {
  const t = String(transcripcion ?? "").trim();
  const preview = t.length > 120 ? `${t.slice(0, 117)}…` : t;
  return (
    (preview ? `Escuché: “${preview}”\n\n` : "Recibí tu audio.\n\n") +
    "No pude sacar correcciones claras de eso.\n" +
    "Dictá de nuevo más despacio (ej: *chasis…, semi…, origen…, destino…*) o *escribilo* por texto.\n" +
    "Si está todo bien, decí *OK*."
  );
}
...
export function mensajeAudioFallidoConfirmacion() {
  return (
    ...
    "Para corregir, dictá los campos o escribilos por texto."
  );
}
