...
export function mensajeAudioNoEntendido(texto, remito) {
  const escuchado = String(texto ?? "").trim();
  const esSaludo = /^(hola|buen[oa]s|hey|eh|che)[.!?\s]*$/i.test(escuchado);
  const corto = escuchado.length > 80 ? `${escuchado.slice(0, 77)}…` : escuchado;
  const pref = corto && !esSaludo ? `Escuché: «${corto}»\n\n` : "";
  if (remito?.estado === "bloqueado") {
    return `${pref}No identifiqué qué hora corregir.\n\nGrabá *2-3 segundos* y decí claro, ej: *salida carga 19:44*.`;
  }
  return `${pref}No identifiqué qué corregir.\n\nGrabá *2-3 segundos*, ej: *Peso 29762* o *Semi ABC123*.`;
}

export function mensajeAudioFallido(remito) {
  if (remito?.estado === "bloqueado") {
    return "No pude entender el audio.\n\nGrabá *2-3 segundos* y decí: *salida carga 19:44* (o escribilo por texto).";
  }
  return "No pude entender el audio.\n\nGrabá *2-3 segundos* con la corrección, o escribila por texto.";
}
