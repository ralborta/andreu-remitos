...
export function parseTodasCorreccionesChofer(texto) {
  ...
  for (const line of raw.split(/\n+/)) {
    const t = line.trim();
    if (!t) continue;
    push(parseCorreccionChofer(t));
    push(parseCorreccionHeuristica(t));
  }
...
export function mensajeCorreccionesAplicadas(correcciones, lectura) {
  ...
  return [header, ...extra, "\n¿Está todo correcto? Respondé *OK* o mandame otro audio con la corrección."].join("\n");
}
...
export async function resolveCorreccionesChofer(texto, opts = {}) {
  ...
  if (!opts.remitoVinculado || !pareceCorreccionRemito(texto)) return [];
