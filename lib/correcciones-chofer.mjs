...
import { esPalabraConfirmacion, normalizarNroRemitoGuia } from "./sanitizar-campos-remito.mjs";
import { ORDEN_CAMPOS, normalizarFecha, normalizarHora } from "./horarios.mjs";
...
function pareceCorreccionRemito(texto) {
  ...
  const keywords =
    /km|kil[oó]met|peso|kilos?|kg\b|destino|origen|procedencia|chofer|conductor|chasis|tractor|semi|acoplado|patente|dominio|gu[ií]a|remito|n[uú]mero|fecha|hora|inicio|final|corrige|corregir|actualiz|cambi|estaba mal|est[aá] mal|no es|en realidad|deber[ií]a|incorrect|faltaba|mal le[ií]|mal el|era |eran |son /i;
...
/** Palabras que el OCR/chofer usa como etiqueta, no como patente. */
const PATENTE_BASURA =
  /^(remolque|acoplado|semi|chasis|tractor|patente|dominio|unidad|camion|camión)$/i;

function esPatenteValida(valor) {
  const v = String(valor ?? "").replace(/\s/g, "").toUpperCase();
  if (!v || v.length < 5 || v.length > 10) return false;
  if (PATENTE_BASURA.test(v) || esPalabraConfirmacion(v)) return false;
  // Debe tener letras y dígitos (AA123BB / MEZ624).
  return /[A-Z]/.test(v) && /\d/.test(v);
}

/** Valor de patente: AA123BB, MEZ624, MEZ 624, AG931YP, etc. */
const VALOR_SEMI = /([A-Za-z]{2}\s?\d{3}\s?[A-Za-z]{2}\b|[A-Za-z]{2,3}\s?\d{3}\b|[A-Za-z0-9+]{6,9}\b)/;
const VALOR_CHASIS = /([A-Za-z]{2}\s?\d{3}\s?[A-Za-z]{2}\b|[A-Za-z0-9+]{6,9}\b)/;

/**
 * Varias patentes en un mensaje: "Remolque MEZ 624 chasis AG931YP".
 */
export function parseCorreccionesPatentesMultiples(texto) {
  if (!texto?.trim()) return [];
  const raw = texto.trim();
  /** @type {Array<{campo: string, valor: string, etiqueta: string, fuente: string}>} */
  const out = [];
  const seen = new Set();

  const reglas = [
    {
      // "semi remolque XXX" antes que "semi XXX" / "remolque XXX"
      re: new RegExp(
        String.raw`\b(?:semi\s*\/?\s*remolque|semi\s+remolque|patente\s*acoplado|acoplado|remolque|semi)\s*(?:es|:)?\s*${VALOR_SEMI.source}`,
        "gi",
      ),
    ...
    {
      re: new RegExp(String.raw`\b(?:chasis|tractor|dominio)\s*(?:es|:)?\s*${VALOR_CHASIS.source}`, "gi"),
      campo: "patente_chasis",
      etiqueta: "Tractor / chasis",
    },
  ];

  for (const { re, campo, etiqueta } of reglas) {
    for (const m of raw.matchAll(re)) {
      if (seen.has(campo)) break;
      const valor = normalizarPatente(m[1]);
      if (!esPatenteValida(valor)) continue;
      seen.add(campo);
      out.push({ campo, valor, etiqueta, fuente: "patentes-multi" });
    }
  }
  return out;
}
...
export function parseCorreccionChofer(texto) {
  ...
  if (chasis) {
    const valor = normalizarPatente(chasis[1]);
  ...
  const semi =
    raw.match(/\bsemi\s*\/?\s*remolque\s*(?:es|:)?\s*([A-Za-z]{2}\s?\d{3}\s?[A-Za-z]{2}|[A-Za-z0-9+]{6,12})\b/i) ??
    raw.match(/\bsemi\s+remolque\s*(?:es|:)?\s*([A-Za-z]{2}\s?\d{3}\s?[A-Za-z]{2}|[A-Za-z0-9+]{6,12})\b/i) ??
    t.match(
      /(?:patente\s*acoplado|acoplado|remolque)\s*(?:es|:)?\s*([a-z]{2}\s?\d{3}\s?[a-z]{2}\b|[a-z]{2,3}\s?\d{3}\b|[a-z0-9+]{6,8})/i,
    ) ??
    t.match(
      /(?:semi)\s*(?:es|:)\s*([a-z]{2}\s?\d{3}\s?[a-z]{2}\b|[a-z]{2,3}\s?\d{3}\b|[a-z0-9+]{6,8})/i,
    ) ??
    t.match(
      /(?:actualiz(?:a|ar|o)|cambiar|cambia)\s*(?:el\s+)?(?:semi|acoplado|remolque)\s*(?:a|por|en)?\s*([a-z]{2}\s?\d{3}\s?[a-z]{2}\b|[a-z]{2,3}\s?\d{3}\b|[a-z0-9+]{6,8})/i,
    ) ??
    raw.match(/\b(?:acoplado|remolque)\s+([A-Za-z]{2}\s?\d{3}\s?[A-Za-z]{2}\b|[A-Za-z]{2,3}\s?\d{3}\b|[A-Za-z0-9+]{6,12})\b/i) ??
    raw.match(/\bsemi\s+([A-Za-z]{2}\s?\d{3}\s?[A-Za-z]{2}\b|[A-Za-z0-9+]{6,12})\b/i);
  if (semi) {
    const valor = normalizarPatente(semi[1]);
    if (esPatenteValida(valor)) {
      return {
        campo: "patente_acoplado",
        valor,
  ...
  const destino = raw.match(/(?:^|\b)(?:el\s+)?destino\s*:?\s*(?:es\s+)?(.+)/i);
  if (destino) {
  ...
  const origen = raw.match(/(?:^|\b)(?:el\s+)?(?:origen|procedencia)\s*:?\s*(?:es\s+)?(.+)/i);
  if (origen) {
  ...
  const ot = raw.match(/(?:^|\b)(?:ot|o\.?\s*t\.?)\s*(?:es|:)?\s*(\d{4,})/i);
  if (ot) {
    return { campo: "ot", valor: ot[1], etiqueta: "OT" };
  }

  const fechaRaw =
    raw.match(/(?:el\s+)?fecha\s*(?:es|:)?\s*(\d{1,2}[\/\-\.\s]\d{1,2}[\/\-\.\s]\d{2,4}|\d{6,8})/i) ??
    raw.match(/\bfecha\s+(\d{1,2}[\/\-\.\s]\d{1,2}[\/\-\.\s]\d{2,4})/i);
  if (fechaRaw) {
    const valor = normalizarFecha(fechaRaw[1]);
    if (valor) return { campo: "fecha", valor, etiqueta: "Fecha" };
  }
...
/** Una sola hora: "salida carga 19:44", "entrada carga 08:30", "hora inicio 6,10", etc. */
export function parseHorarioUnico(texto) {
  if (!texto?.trim()) return null;
  const raw = texto.trim();
  const conSep = raw.match(/\b(\d{1,2})\s*[,:h.]\s*(\d{2})\b/);
  const horaRaw =
    conSep ??
    raw.match(/\b(\d{3,4})\s*hs?\b/i) ??
    raw.match(/\b(\d{4})\b/);
  if (!horaRaw) return null;
  const hora = normalizarHora(
    conSep ? `${conSep[1]}:${conSep[2]}` : horaRaw[1],
  );
  if (!hora) return null;
  ...
  const map = [
    ...
    { re: /\b(?:salida\s*(?:de\s*)?descarga|descarga\s*salida|fin\s*(?:de\s*)?descarga|descarga\s*fin)\b/, campo: "descarga_fin", etiqueta: "Descarga — hora fin" },
    { re: /\bhora\s*(?:de\s*)?inicio\b/, campo: "carga_entrada", etiqueta: "Carga — hora entrada" },
    { re: /\bhora\s*(?:de\s*)?(?:final|fin)\b/, campo: "descarga_fin", etiqueta: "Descarga — hora fin" },
  ];
...
/**
 * Varias horas etiquetadas en un mensaje:
 * "HORA DE INICIO 6,10 HORA FINAL 10,38"
 */
export function parseHorariosEtiquetados(texto) {
  if (!texto?.trim()) return [];
  const raw = texto.trim();
  /** @type {Array<{campo: string, valor: string, etiqueta: string, fuente: string}>} */
  const out = [];
  ...
  const reglas = [
    {
      re: /\bhora\s*(?:de\s*)?inicio\s*:?\s*(\d{1,2})\s*[,:h.]\s*(\d{2})\b/gi,
      campo: "carga_entrada",
      etiqueta: "Carga — hora entrada",
    },
    {
      re: /\bhora\s*(?:de\s*)?(?:final|fin)\s*:?\s*(\d{1,2})\s*[,:h.]\s*(\d{2})\b/gi,
      campo: "descarga_fin",
      etiqueta: "Descarga — hora fin",
    },
    {
      re: /\b(?:entrada\s*(?:de\s*)?carga|carga\s*entrada)\s*:?\s*(\d{1,2})\s*[,:h.]\s*(\d{2})\b/gi,
      campo: "carga_entrada",
      etiqueta: "Carga — hora entrada",
    },
    {
      re: /\b(?:salida\s*(?:de\s*)?carga|carga\s*salida)\s*:?\s*(\d{1,2})\s*[,:h.]\s*(\d{2})\b/gi,
      campo: "carga_salida",
      etiqueta: "Carga — hora salida",
    },
    {
      re: /\b(?:llegada\s*(?:a\s*)?descarga|descarga\s*llegada)\s*:?\s*(\d{1,2})\s*[,:h.]\s*(\d{2})\b/gi,
      campo: "descarga_llegada",
      etiqueta: "Descarga — hora llegada",
    },
    {
      re: /\b(?:inicio\s*(?:de\s*)?descarga|descarga\s*inicio)\s*:?\s*(\d{1,2})\s*[,:h.]\s*(\d{2})\b/gi,
      campo: "descarga_inicio",
      etiqueta: "Descarga — hora inicio",
    },
    {
      re: /\b(?:salida\s*(?:de\s*)?descarga|descarga\s*(?:salida|fin)|fin\s*(?:de\s*)?descarga)\s*:?\s*(\d{1,2})\s*[,:h.]\s*(\d{2})\b/gi,
      campo: "descarga_fin",
      etiqueta: "Descarga — hora fin",
    },
  ];
  ...
  for (const { re, campo, etiqueta } of reglas) {
    for (const m of raw.matchAll(re)) {
      const key = `_hora_${campo}`;
      if (seen.has(key)) continue;
      const hora = normalizarHora(`${m[1]}:${m[2]}`);
      if (!hora) continue;
      seen.add(key);
      out.push({ campo: key, valor: hora, etiqueta, fuente: "horarios-etiq" });
    }
  }
  return out;
}
...
export function parseCorreccionHeuristica(texto) {
  ...
  if (corrige) {
    ...
    if (tipo.includes("fecha")) {
      const valor = normalizarFecha(resto.match(/(\d{1,2}[\/\-\.\s]\d{1,2}[\/\-\.\s]\d{2,4}|\d{6,8})/)?.[1] ?? resto);
      if (valor) return { campo: "fecha", valor, etiqueta: "Fecha", fuente: "heuristica" };
    }
  }
...
/** Origen/destino en la misma línea: "ORIGEN NEXA DESTINO PAMPA PAD 12/13". */
function parseOrigenDestinoInline(raw) {
  if (!/(?:origen|procedencia|destino)\b/i.test(raw)) return [];
  /** @type {Array<{campo: string, valor: string, etiqueta: string, fuente: string}>} */
  const out = [];
  const origen = raw.match(
    /(?:^|\b)(?:origen|procedencia)\s*:?\s*(?:es\s+)?(.+?)(?=\s+destino\b|\s+salida\s+(?:de\s+)?(?:carga|descarga)\b|\s+horarios?\b|$)/i,
  );
  if (origen) {
    const valor = limpiarValor(origen[1]);
    if (valor.length >= 2) out.push({ campo: "origen", valor, etiqueta: "Origen", fuente: "inline" });
  }
  const destino = raw.match(
    /(?:^|\b)destino\s*:?\s*(?:es\s+)?(.+?)(?=\s+salida\s+(?:de\s+)?(?:carga|descarga)\b|\s+horarios?\b|$)/i,
  );
  if (destino) {
    const valor = limpiarValor(destino[1]);
    if (valor.length >= 2) out.push({ campo: "destino", valor, etiqueta: "Destino", fuente: "inline" });
  }
  return out;
}
...
export function parseTodasCorreccionesChofer(texto) {
  ...
  for (const part of partesMensajeCorreccion(raw)) {
    ...
    for (const h of parseHorariosEtiquetados(part)) push(h);
  }
  ...
  if (out.length === 0) {
    ...
    for (const h of parseHorariosEtiquetados(raw)) push(h);
  } else {
    for (const h of parseHorariosCorreccion(raw)) push(h);
    for (const h of parseHorariosEtiquetados(raw)) push(h);
  }

  for (const c of parseCorreccionesPatentesMultiples(raw)) push(c);
  for (const c of parseOrigenDestinoInline(raw)) push(c);
...
export function buildPatchFromCorrecciones(tenant, correcciones, remitoDatos = {}) {
  ...
  const fechaCorreccion = campos.find((c) => c.campo === "fecha");
  if (fechaCorreccion) {
    const fechaNorm = normalizarFecha(String(fechaCorreccion.valor)) ?? String(fechaCorreccion.valor);
    const existingHorarios = remitoDatos?.horarios?.horarios;
    if (existingHorarios && typeof existingHorarios === "object") {
      /** @type {Record<string, { fecha?: string|null, hora?: string|null }>} */
      const horariosRaw = {};
      for (const [k, slot] of Object.entries(existingHorarios)) {
        horariosRaw[k] = { ...(slot ?? {}), fecha: fechaNorm };
      }
      patch.horarios = { horarios: horariosRaw, fecha_remito: fechaNorm };
    }
  }
  ...
  if (keysHoras.length > 0) {
    ...
    const existing = d.horarios?.horarios ?? {};
    /** @type {Record<string, { fecha: string|null, hora: string }>} */
    const horariosRaw = {};
    for (const k of ORDEN_CAMPOS) {
      const prev = existing[k];
      if (prev?.hora || horas[k]) {
        horariosRaw[k] = {
          fecha: prev?.fecha ?? fechaBase,
          hora: horas[k] ?? prev?.hora ?? null,
        };
      }
    }
...
export function mensajeCorreccionesAplicadas(correcciones, lectura) {
  const d = lectura ?? {};
  const lineas = correcciones.map((c) => `• *${c.etiqueta}*: ${c.valor}`);
  const header =
  ...
  if (d.ot != null && String(d.ot).trim() !== "") extra.push(`OT: ${d.ot}`);
  if (d.km_final != null) extra.push(`KM final: ${d.km_final}`);

  return [header, ...extra, "\nSi está todo bien respondé *OK* (texto o audio). Si falta algo, *escribí* la corrección."].join("\n");
}
...
export function patchCorreccionRemito(tenant, correccion) {
  ...
  if (campo === "patente_chasis" || campo === "patente_acoplado") {
    if (esPalabraConfirmacion(valor)) return {};
    valor = normalizarPatente(valor);
    if (!esPatenteValida(valor)) return {};
  }
  if (campo === "nro_remito" || campo === "nro_guia") {
    const n = normalizarNroRemitoGuia(valor, { tenant });
    if (!n) return {};
    valor = n;
  }
  /** @type {Record<string, string|number|boolean>} */
  ...
  switch (campo) {
    ...
    case "patente_chasis":
      ...
      else if (tenant === "tsb" || tenant === "mye") assign(["chasis", "patente_chasis"], valor);
    ...
    case "patente_acoplado":
      ...
      else if (tenant === "tsb" || tenant === "mye") assign(["acoplado", "patente_acoplado"], valor);
      else if (tenant === "corina") assign(["semi", "patente_acoplado"], valor);
      else patch.patente_acoplado = valor;
      break;
    case "chofer":
      if (tenant === "tsb" || tenant === "mye" || tenant === "corina") assign(["conductor", "chofer"], valor);
    ...
    case "destino":
      ...
      break;
    case "origen":
      if (tenant === "tsb" || tenant === "mye") patch.procedencia = valor;
    ...
    case "peso_kg":
      ...
      break;
    case "nro_remito":
      if (tenant === "tsb" || tenant === "mye") patch.nro_guia = valor;
      else patch.nro_remito = valor;
      break;
    case "ot":
      patch.ot = String(valor).replace(/\D/g, "") || valor;
      break;
    case "fecha": {
      const fechaNorm = normalizarFecha(String(valor)) ?? String(valor);
      if (tenant === "tsb" || tenant === "mye") assign(["fecha_guia", "fecha_remito", "fecha"], fechaNorm);
      else assign(["fecha_remito", "fecha"], fechaNorm);
      break;
    }
    default:
      patch[campo] = valor;
