...
const VALOR_SEMI = /([A-Za-z]{2,3}\s?\d{3}\b|[A-Za-z]{2}\s?\d{3}\s?[A-Za-z]{2}\b|[A-Za-z0-9+]{6,9}\b)/;
...
export function parseCorreccionesPatentesMultiples(texto) {
  ...
  const reglas = [
    {
      re: new RegExp(String.raw`\b(?:semi\s*\/\s*remolque|semi|acoplado|remolque)\s*(?:es|:)?\s*${VALOR_SEMI.source}`, "gi"),
  ...
  for (const { re, campo, etiqueta } of reglas) {
    for (const m of raw.matchAll(re)) {
      ...
      if (!valor || valor.length < 5 || esPalabraConfirmacion(valor)) continue;
      seen.add(campo);
...
export function parseCorreccionChofer(texto) {
  ...
  if (chasis) {
    ...
    if (!esPalabraConfirmacion(valor)) {
      return {
        campo: "patente_chasis",
        valor,
        etiqueta: "Tractor / chasis",
  ...
  const semi =
    raw.match(/\bsemi\s*\/\s*remolque\s+([A-Za-z0-9+]{6,12})\b/i) ??
    t.match(
      /(?:semi|acoplado|patente\s*acoplado|remolque)\s*(?:es|:)?\s*([a-z]{2,3}\s?\d{3}\b|[a-z]{2}\s?\d{3}\s?[a-z]{2}\b|\w{6,8})/i,
    ) ??
    t.match(
      /(?:actualiz(?:a|ar|o)|cambiar|cambia)\s*(?:el\s+)?(?:semi|acoplado|remolque)\s*(?:a|por|en)?\s*([a-z]{2,3}\s?\d{3}\b|[a-z]{2}\s?\d{3}\s?[a-z]{2}\b|\w{6,8})/i,
    ) ??
    raw.match(/\b(?:acoplado|remolque)\s+([A-Za-z]{2,3}\s?\d{3}\b|[A-Za-z0-9+\s]{6,12})\b/i) ??
...
export function parseHorarioUnico(texto) {
  ...
  const raw = texto.trim();
  const horaRaw =
    raw.match(/\b(\d{1,2})\s*[:h.]\s*(\d{2})\b/) ??
    raw.match(/\b(\d{3,4})\s*hs?\b/i) ??
    raw.match(/\b(\d{4})\b/);
  if (!horaRaw) return null;
  const horaInput =
    horaRaw[3] ?? (horaRaw[2] != null ? `${horaRaw[1]}:${horaRaw[2]}` : horaRaw[1]);
  const hora = normalizarHora(horaInput);
  if (!hora) return null;
...
export function patchCorreccionRemito(tenant, correccion) {
  ...
  if (campo === "patente_chasis" || campo === "patente_acoplado") {
    ...
    if (!valor || valor.length < 5 || esPalabraConfirmacion(valor)) return {};
  }
