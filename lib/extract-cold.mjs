...
import { esEtiquetaCampoRemito, normalizarNroRemitoGuia } from "./sanitizar-campos-remito.mjs";
...
export function extraerBeraldi(texto) {
  ...
  const nroRemito =
    cap(t, /REMITO\s*N[°º]?\s*((?:\d{5}[-\s]?)?\d{5,}(?:\s*[-–—]\s*\d{1,2})?)/i) ??
    cap(t, /N[°º]?\s*((?:00009[-\s]?)?\d{5,}(?:\s*[-–—]\s*\d{1,2})?)/i);
  ...
  return {
    ...
    ot: cap(t, /[OΟОо]\s*[.\-]?\s*[TТт]\s*[.:]?\s*(\d{4,8})/i)
      ?? cap(t, /(?:^|\n)\s*Not\s*[.:]?\s*(\d{4,8})/im)
      ?? cap(t, /\bOT\s+(\d{4,8})\b/i),
    ...
    km_inicial: cap(t, /Km\.?\s*Inic\w*[:\s]*([\d.,]+)/i) ?? cap(t, /Km\s*[Ii]nicial:?\s*([\d\.]+)/i),
    km_final: cap(t, /Km\.?\s*Fin(?:al)?\w*[:\s]*([\d.,]+)/i) ?? cap(t, /Km\s*[Ff]inal\.?\s*([\d\.]+)/i),
    horarios,
  };
}
...
function limpiarLugarBeraldi(valor) {
  ...
  if (/hora\s+de\s+(llegada|entrada|salida|inicio|fin)/i.test(s)) return null;
  if (/^hora[_\s]?(ent|sal|ini|fin|lleg)/i.test(s)) return null;
  return s;
}
...
function valorLugarMye(raw) {
  if (raw == null) return null;
  let s = String(raw)
    .replace(/\s+/g, " ")
    .trim()
    .replace(/\s*(DESTINO\s*\/?\s*POZO|LUGAR\s+DE\s+CARGA|CHOFER|EQUIPO|CAMION|ACOPLADO|PRODUCTO|CANTIDAD|PESO).*$/i, "")
    .replace(/[:.|]+$/g, "")
    .trim();
  if (!s || s.length < 2) return null;
  if (esEtiquetaCampoRemito(s)) return null;
  return s;
}

/**
 * Remito MyESA: LUGAR DE CARGA = origen (procedencia), DESTINO/POZO = destino.
 * El valor puede ir en la misma línea o en la siguiente (manuscrito).
 */
export function extraerMye(texto) {
  const t = String(texto ?? "");
  const horarios = parsearHorarios(limpiar(t));

  function capCampo(labelRe, stops) {
    const stop = stops.join("|");
    const sameLine = t.match(
      new RegExp(`${labelRe}\\s*:?\\s*([^\\n]{0,48}?)(?=\\s*(?:${stop})|$)`, "i"),
    );
    const same = valorLugarMye(sameLine?.[1]);
    if (same) return same;
    const nextLine = t.match(new RegExp(`${labelRe}\\s*:?\\s*\\n\\s*([^\\n]{2,40})`, "i"));
    return valorLugarMye(nextLine?.[1]);
  }

  const procedencia = capCampo("LUGAR\\s+DE\\s+CARGA", [
    "DESTINO",
    "CHOFER",
    "EQUIPO",
    "CAMION",
  ...
  const destino = capCampo("DESTINO\\s*\\/?\\s*POZO", [
    "CHOFER",
    "EQUIPO",
    "CAMION",
    "ACOPLADO",
  ...
  const pesoRaw =
    cap(t, /Peso\s+Neto\s*:?\s*([\d.,]+)/i) ??
    cap(t, /CANTIDAD[\s\S]{0,30}?([\d.,]{4,})/i) ??
    cap(t, /PESO\s*:?\s*([\d.,]{4,})/i);

  const camion = cap(t, /CAMION\s*:?\s*([A-Z0-9]{2,3}\s*\d{3}\s*[A-Z0-9]{0,3})/i);
  const acoplado = cap(t, /ACOPLADO\s*:?\s*([A-Z0-9]{2,3}\s*\d{3}\s*[A-Z0-9]{0,3})/i);

  return {
    tenant: "mye",
    nro_guia: normalizarNroRemitoGuia(
      cap(t, /N[°º.]?\s*Remito\s*:?\s*([\d\-–—]+)/i) ?? cap(t, /N[°º]?\s*0*(\d{5,})/i),
    ),
    fecha_guia: normalizarFecha(cap(t, /Fecha\s*:?\s*(\d{1,2}\s*[\/.\-]\s*\d{1,2}\s*[\/.\-]\s*\d{2,4})/i)),
    procedencia,
    destino,
    chasis: camion,
    acoplado,
    conductor: cap(t, /CHOFER\s*:?\s*([A-Za-záéíóúñÁÉÍÓÚÑ\s.]+?)(?=\s*(?:EQUIPO|CAMION|ACOPLADO|PRODUCTO|$))/i),
    peso_kg: normalizarPeso(pesoRaw),
    horarios,
  };
}
...
export function extraerFrio(texto) {
  ...
  const datos =
    tenant === "mye" ? extraerMye(texto) : tenant === "tsb" ? extraerTSB(texto) : extraerBeraldi(texto);
