...
/**
 * Auto-detecta TSB / MyESA / Beraldi: heurística OCR + dual processor + IA si hace falta.
 */
async function autoDetectarTenant(buffer, filename, opts = {}) {
  ...
  if (detectado.tenant === "mye" && process.env.DOCUMENT_AI_CUSTOM_MYE_ID) {
    const { ocr, lectura } = await lecturaDesdeTenant(buffer, filename, "mye");
    lectura.tenant = "mye";
    lectura.resumen = lectura.resumen ?? {};
    lectura.resumen.tenant_detectado = "mye";
    lectura.resumen.tenant_razon = "ocr_heuristica";
    lectura.resumen.tenant_confianza = detectado.confianza;
    return { ocr, lectura, eleccion: { tenant: "mye", razon: "ocr_heuristica", confianza: detectado.confianza } };
  }
  ...
  /** @type {ReturnType<typeof extraerBeraldiFoundation> | undefined} */
  let lecturaBeraldi;
  ...
  if (ia?.tenant === "mye" && process.env.DOCUMENT_AI_CUSTOM_MYE_ID) {
    const { ocr, lectura } = await lecturaDesdeTenant(buffer, filename, "mye");
    lectura.tenant = "mye";
    lectura.resumen = lectura.resumen ?? {};
    lectura.resumen.tenant_detectado = "mye";
    lectura.resumen.tenant_razon = "ia";
    lectura.resumen.tenant_confianza = ia.confianza;
    return { ocr, lectura, eleccion: { tenant: "mye", razon: "ia", confianza: ia.confianza } };
  }
...
export async function leerRemito(buffer, opts = {}) {
  ...
  if (forzado === "corina" || forzado === "tsb" || forzado === "beraldi" || forzado === "mye") {
    ...
    // El papel manda si la señal es clara (evita WA pegado a TSB con remito M&E)
    const papelClaro =
      detectadoDoc.tenant !== "desconocido" &&
      detectadoDoc.tenant !== forzado &&
      (detectadoDoc.margen >= 6 || (detectadoDoc.scores?.[detectadoDoc.tenant] ?? 0) >= 8);

    if (papelClaro) {
      const tenantPapel = detectadoDoc.tenant;
      ({ ocr, lectura } = await lecturaDesdeTenant(buffer, opts.filename, tenantPapel));
      lectura.tenant = tenantPapel;
      lectura.resumen = lectura.resumen ?? {};
      lectura.resumen.tenant_detectado = tenantPapel;
      lectura.resumen.tenant_razon = "ocr_override_forzado";
      lectura.resumen.advertencia_tenant = {
        esperado: forzado,
        detectado: tenantPapel,
        mensaje: `Se forzaba ${forzado.toUpperCase()} pero el papel es ${tenantPapel.toUpperCase()} — se usó el documento`,
      };
    } else if (detectadoDoc.tenant !== "desconocido" && detectadoDoc.tenant !== forzado && detectadoDoc.margen >= 4) {
      lectura.resumen = lectura.resumen ?? {};
      lectura.resumen.advertencia_tenant = {
        esperado: forzado,
