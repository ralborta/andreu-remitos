...
import { buildPlanillaTmsQmResult } from "./planilla-tms-qm.mjs";
...
export async function buildPlanillaBeraldi(opts = {}) {
  ...
  // FechaInicio ascendente: menor (más vieja) arriba → mayor abajo
  filtrados.sort(compararRemitosPorFechaAsc);
  filtrados = filtrados.slice(0, limit);

  if (formato === "qm") {
    return buildPlanillaTmsQmResult({
      tenant: "beraldi",
      filtrados,
      ctx,
      tmsQmHooks: {
        origenFields: ["origen", "procedencia"],
        destinoFields: ["destino", "destino_nombre", "destino_locacion"],
        choferFields: ["chofer"],
        tractorFields: ["tractor", "patente_chasis", "chasis"],
        semiFields: ["semi", "patente_acoplado", "acoplado"],
        findChofer: (remito, d) => findChofer(ctx.choferes, remito.telefono_chofer, d.chofer),
        formatoChofer: formatoChoferBeraldi,
      },
      metaExtra: { desde: desde ?? null, hasta: hasta ?? null },
    });
  }
