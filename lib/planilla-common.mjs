...
/**
 * Columnas A–Y — plantilla de importación Delfos / QuadMy.
 * Headers exactos (guión bajo, sin espacios) — si cambian, Delfos no importa la 1ª fila.
 */
...
export const PLANILLA_DIARIA_COLUMNS = [
  ...
  { key: "patente_tractor", header: "PATENTE TRACT", width: 108 },
  { key: "int_tractor", header: "INT TRACTO", width: 96 },
...
/** Fecha del papel (guía/remito) sin fallback a created_at — para filtrar reportes. */
export function fechaPapelRemito(remito) {
  const d = remito?.datos ?? {};
  return (
    normalizarFecha(d.fecha_guia) ??
    normalizarFecha(d.fecha) ??
    normalizarFecha(d.fecha_remito) ??
    normalizarFecha(d.horarios?.fecha_remito) ??
    null
  );
}

export function parseFechaRemito(remito) {
  // Solo fechas normalizadas/válidas (rechaza años OCR absurdos tipo 8126).
  return fechaPapelRemito(remito) ?? remito.created_at?.slice(0, 10) ?? "";
}

/** Normaliza a YYYY-MM-DD para ordenar / filtrar (acepta ISO o DD/MM/YYYY). */
export function isoFecha(value) {
...
/** Orden cronológico ascendente por fecha del remito (FechaInicio). */
export function compararRemitosPorFechaAsc(a, b) {
  const fa = isoFecha(parseFechaRemito(a)) || "9999-99-99";
  const fb = isoFecha(parseFechaRemito(b)) || "9999-99-99";
  if (fa !== fb) return fa.localeCompare(fb);
  return String(a.created_at ?? "").localeCompare(String(b.created_at ?? ""));
}

/** Orden cronológico descendente por FechaInicio (más reciente primero). */
export function compararRemitosPorFechaDesc(a, b) {
  const fa = isoFecha(parseFechaRemito(a)) || "0000-00-00";
  const fb = isoFecha(parseFechaRemito(b)) || "0000-00-00";
...
/**
 * Incluye el remito solo si la FechaInicio del papel (guía/remito) cae en el rango.
 * No usa created_at/updated_at: si no, al filtrar 9–11/8 entraban viajes del 7/8
 * solo porque se cargaron esos días.
 */
export function remitoEnRango(remito, { desde, hasta }) {
  if (!desde && !hasta) return true;
  const iso = isoFecha(fechaPapelRemito(remito));
  if (!iso) return false;
  return fechaEnRango(iso, { desde, hasta });
}

/** Clave de documento para detectar cargas duplicadas del mismo remito/guía. */
export function claveDocumentoRemito(remito) {
  const d = remito?.datos ?? {};
  const raw =
    remito?.tenant === "corina"
      ? d.nro_remito ?? d.nro_guia ?? ""
      : d.nro_guia ?? d.nro_remito ?? "";
  const digits = String(raw).replace(/\D/g, "");
  if (digits) return digits;
  const cliente = String(d.remito_cliente ?? "").replace(/\D/g, "");
  return cliente || null;
}

function esMasRecienteCarga(a, b) {
...
/**
 * En planilla: si el mismo nro de guía/remito está cargado varias veces,
 * se queda la última carga (criterio actual). Más adelante se rechazará al ingresar.
 */
export function dedupeRemitosPlanilla(remitos) {
  const porClave = new Map();
  const sinClave = [];
  for (const remito of remitos) {
    const clave = claveDocumentoRemito(remito);
    if (!clave) {
      sinClave.push(remito);
      continue;
    }
    const prev = porClave.get(clave);
    if (!prev || esMasRecienteCarga(remito, prev)) porClave.set(clave, remito);
  }
...
export async function buildPlanillaFromRemitos({
  ...
  formato,
  mapRemito,
  columnas: columnasOverride,
}) {
  const esProforma = formato === "proforma";
  const columnas = columnasOverride ?? columnasParaFormato(formato);
  const filas = [];
  let nroViaje = 1;
