...
export async function turnoAgenteViajes({
  ...
  if (ia) {
    ...
    // Número del cliente gana siempre sobre el LLM (evita tomar la opción 1 por error).
    const seleccionHeuristica = fase === "propuesta" ? parseSeleccionOpcion(texto) : null;
    let seleccion =
      seleccionHeuristica != null
        ? seleccionHeuristica
        : ia.seleccion != null && Number.isFinite(Number(ia.seleccion))
          ? Number(ia.seleccion)
          : null;

    // Guardrails de negocio: no inventar acciones
    if (fase === "propuesta" && seleccion != null) {
      accion = "reservar";
    } else if (fase === "propuesta" && (intent === "confirmar" || accion === "reservar")) {
      accion = "reservar";
    ...
    return {
      datos: merged,
      faltan,
      intent: seleccion != null ? "seleccion" : intent,
      seleccion,
      accion,
      mensaje,
      fuente: ia.motor || "ia",
...
async function callAgenteJson({
  ...
  const hechosConsulta = consulta
    ? {
        ...
        fuente_flota: consulta.fuente || null,
        match_flexible: consulta.match_flexible || false,
...
Trabajás 100% por comprensión del lenguaje: no dependés de formularios ni palabras clave.
...
- Extraé TODO lo que el cliente ya dijo (ciudad destino, origen, tn, tipo de carga, fecha/hora) en datos_patch.
- No pidas datos que ya estén en DATOS_ACTUALES ni los que acabás de extraer del mensaje.
...
- Si el mensaje es un pedido inicial ("necesito un viaje a Córdoba…"), reconocelo, guardá lo que haya y pedí solo lo que falte.
- Si falta info, pedí lo esencial. Si está completa, avisá que vas a consultar disponibilidad.
- Si hay opciones en HECHOS_CONSULTA, explicalas con fecha/hora reales y pedí confirmación. NUNCA digas "no hay" si hay opciones.
- Si sin_cupo=true, pedí otra fecha/hora; un "sí" del cliente en ese contexto es "buscá alternativas / pasame otra fecha", NO reservar.
- Si el cliente confirma una propuesta concreta (sí / me sirve / dale / opción N) Y hay PROPUESTA_ACTIVA, accion=reservar.
...
datos_patch: campos que el mensaje aporte (aunque sea parcial). Si dudás, null (no adivines ciudades inventadas).
...
export async function redactarPropuestaDisponibilidad({
  ...
  // Con alternativas: lista numerada determinística (mismo orden que se guarda en propuesta.opciones).
  // Evita que el LLM reordene y el cliente elija "2" apuntando a otra hora.
  const alts = consulta?.propuesta
    ? [consulta.propuesta, ...(consulta.alternativas ?? [])].slice(0, 4)
    : [];
  if (alts.length > 1) {
    return mensajePropuestaReserva(consulta, datos);
  }
...
export function mensajePropuestaReserva(consulta, datos = {}) {
  ...
  return (
    ...
    `¿Cuál te sirve? Respondé solo el *número* (1, 2, 3…) y genero la reserva.`
  );
}
...
export function parseSeleccionOpcion(texto) {
  const t = String(texto ?? "").trim();
  // Solo el dígito / "opción N" — 1-based (igual que el LLM: 1..4)
  const m = t.match(/^(?:opci[oó]n\s*|la\s*|n[uú]mero\s*|#\s*)?([1-4])\s*[.)]?\s*$/i);
  if (!m) return null;
  return Number(m[1]);
}

/** Índice 0-based seguro a partir de selección 1-based. */
export function indiceOpcion(seleccion, total = 4) {
  const n = Number(seleccion);
  if (!Number.isFinite(n) || n < 1 || n > total) return null;
  return n - 1;
}

/**
 * Resuelve qué slot de la propuesta eligió el cliente.
 * - Número 1..N → esa opción (el dígito del mensaje gana).
 * - "sí" con 1 sola opción → esa.
 * - "sí" con varias → pedir número (no asumir la 1).
 */
export function resolverSlotPropuesta({
  opciones = [],
  seleccion = null,
  texto = "",
  elegidaPendiente = null,
} = {}) {
  const lista = Array.isArray(opciones) ? opciones.filter(Boolean) : [];
  if (!lista.length) {
    return { slot: null, seleccion: null, motivo: "sin_opciones" };
  }

  const selMensaje = parseSeleccionOpcion(texto);
  const sel =
    selMensaje ??
    (seleccion != null && Number.isFinite(Number(seleccion)) ? Number(seleccion) : null);
  const idx = indiceOpcion(sel, lista.length);
  if (idx != null) {
    return { slot: lista[idx], seleccion: sel, motivo: "numero" };
  }

  if (esConfirmacionCliente(texto)) {
    if (lista.length === 1) {
      return { slot: lista[0], seleccion: 1, motivo: "unica" };
    }
    // Varias opciones: nunca asumir la 1 (bug de demo Gonzalo / 16:00).
    return { slot: null, seleccion: null, motivo: "pedir_numero" };
  }

  if (elegidaPendiente && lista.length === 1) {
    return { slot: lista[0], seleccion: 1, motivo: "pendiente_unica" };
  }

  return { slot: null, seleccion: null, motivo: "sin_match" };
}

export function mensajePedirNumeroOpcion(opciones = []) {
  const lista = (opciones || [])
    .slice(0, 4)
    .map((s, i) => `${i + 1}) ${formatearFechaCorta(s.fecha)} ${s.hora}`)
    .join("; ");
  return lista
    ? `Tenés estas opciones: ${lista}. Respondeme solo el *número* (1, 2, 3…) para confirmar.`
    : `Decime el *número* de la opción que te sirve (1, 2, 3…).`;
}
...
export async function interpretarMensajeViaje(texto, { pendingDatos = {}, remitente, log } = {}) {
  ...
  return {
    ...
    parece_solicitud: Boolean(
      turno.datos?.origen || turno.datos?.destino || turno.intent === "dato" || !turno.faltan?.length,
    ),
  };
}
