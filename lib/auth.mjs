...
export const ROLES = /** @type {const} */ (["operador", "supervisor", "administrador"]);

/** @typedef {"operador" | "supervisor" | "administrador"} RolUsuario */
...
/** Supervisor y administrador pueden eliminar remitos. */
export function canDeleteRemitos(session) {
  return session?.rol === "administrador" || session?.rol === "supervisor";
}
...
export function isPublicApiPath(pathname, method) {
  ...
  if (pathname === "/api/vincular/whatsapp/qr" && method === "GET") return true;
  if (pathname.startsWith("/api/tracking/public/")) return true;
  return false;
}

/** Supervisor y administrador pueden mutar parámetros maestros. */
