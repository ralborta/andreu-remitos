...
import { corregirPatenteMercosur, patenteParaMatch } from "./patente-ocr.mjs";
...
export function puntajePatente(unidad, patenteOcr) {
  ...
  const o = patenteParaMatch(patenteOcr);
  const oRaw = patentePlanilla(patenteOcr);
  if (!p || !o) return 0;
  if (p === o || p === oRaw) return 100;
  ...
  // Mismo prefijo de letras (AH…) — típico OCR con dígitos mal leídos
  if (p.length >= 4 && o.length >= 4 && p.slice(0, 2) === o.slice(0, 2)) {
    score = Math.max(score, 68);
  }

  const dist = levenshtein(p, o);
  const sim = 1 - dist / Math.max(p.length, o.length);
  if (sim >= 0.65 && Math.max(p.length, o.length) >= 6) score = Math.max(score, sim * 85);

  // 1 solo dígito/letra distinto, misma longitud (típico 8↔0, 1↔7)
  if (dist === 1 && p.length === o.length && p.length >= 6) {
    score = Math.max(score, 94);
  }

  const distRaw = levenshtein(p, oRaw);
  const simRaw = 1 - distRaw / Math.max(p.length, oRaw.length);
  if (simRaw >= 0.65 && Math.max(p.length, oRaw.length) >= 6) {
    score = Math.max(score, simRaw * 80);
  }
  if (distRaw === 1 && p.length === oRaw.length && p.length >= 6) {
...
export function findUnidad(unidades, patente, tipo) {
  ...
  const exact = pool.find((u) => {
    const up = patentePlanilla(u.patente);
    return up === patentePlanilla(patente) || up === patenteParaMatch(patente);
  });
  if (exact) return exact;

  const ocrCorregida = corregirPatenteMercosur(patente);
  if (ocrCorregida && ocrCorregida !== patentePlanilla(patente)) {
    const porCorreccion = pool.find((u) => patentePlanilla(u.patente) === ocrCorregida);
    if (porCorreccion) return porCorreccion;
  }

  // Preferencia: un único candidato a distancia 1 (OCR 8↔0)
  const o = patenteParaMatch(patente);
  if (o && o.length >= 6) {
    const casi = pool.filter((u) => {
      const up = patentePlanilla(u.patente);
      return up.length === o.length && levenshtein(up, o) === 1;
    });
    if (casi.length === 1) return casi[0];
  }

  const candidatos = pool.map((u) => ({ item: u, score: puntajePatente(u, patente) }));
  return mejorCandidato(candidatos, 68, 6)?.item ?? null;
}
...
export function canonicalizarConMaestros(datos, tenant, ctx = {}) {
  const {
    ...
    /** Si true, no reemplaza patentes/chofer por fuzzy match (edición manual del operador). */
    respetarEdicionManual = false,
  } = ctx;

  if (!datos) return datos;

  let d = { ...datos };
  const respetarManual = respetarEdicionManual || d._editado_manual === true;
  ...
  if (unidades.length && !respetarManual) {
  ...
  if (choferes.length && !choferDesdeTelefono && !respetarManual) {
    const nombreOcr = d.conductor ?? d.chofer;
    const chofer = findChoferPorNombre(choferes, nombreOcr);
    if (chofer?.nombre) {
      if (tenant === "tsb" || tenant === "mye" || tenant === "corina") d.conductor = chofer.nombre;
