...
export async function interpretarRespuestaChoferEta(texto, { pending, log } = {}) {
  ...
  if (parsed) {
    const demora = pareceDemoraChofer(t) && pending?.estado === "en_ruta";
    return {
  ...
  // en_ruta sin tiempo claro — pedir aclaración de demora
  return {
    intent: "pedir_eta",
    minutos: null,
    etaTexto: null,
    mensaje:
      `¿Cuánto estimás ahora de demora o llegada? (ej: *15 min más* o *llego en 40 min*)`,
    fuente: "heuristica",
  };
}
...
async function parseEtaChoferConIA(texto, { pending, log } = {}) {
  ...
  const prompt = `Sos el agente de Confirmación de Destinos. Hablás con el *chofer* por WhatsApp.
Estado del pedido: ${pending?.estado ?? "desconocido"}
Destino: ${pending?.formatted_address ?? "—"}
Cliente: ${pending?.cliente ?? "—"}
ETA previo (si hay): ${pending?.eta_texto ?? "ninguno"}
...
Clasificá:
- "eta": da un tiempo de llegada (primera estimación).
- "demora": avisa retraso o actualiza el estimado cuando ya estaba en ruta.
- "pedir_eta": no se entiende el tiempo; pedí un ejemplo claro en "mensaje".
- "chat": pregunta/charla; respondé breve en "mensaje" y pedí el ETA si falta.

Devolvé minutos (entero) cuando puedas, y eta_texto humano en español (ej: "30 minutos", "1 hora").`;
  ...
  try {
    ...
    log?.info?.({ intent, minutos, fuente: "ia" }, "Destinos ETA chofer IA");
