...
export function mensajePedirCausaIncidencia({
  tipoHint,
  viajeRef,
  proactivo = false,
  direccion = null,
  ...
  if (proactivo) {
    // Flujo principal: el agente detecta parada (ubicación) y pregunta al chofer.
    let donde = "";
    if (direccion) donde += `\n📍 ${direccion}`;
    if (mapsLink) donde += `\n${mapsLink}`;

    return (
      `Hola 👋 Detectamos que estás *parado*` +
      (viajeRef && viajeRef !== "—" ? ` en el viaje *${viajeRef}*` : "") +
      (tipoTxt && tipoTxt !== "Parada no prevista" ? ` (posible *${tipoTxt}*)` : "") +
      `.` +
      donde +
      `\n\n` +
      `¿*Por qué estás parado?* Contame en un mensaje (ej: tránsito, entrega, pinchazo, control, mecánico…).\n` +
...
/** Segundo aviso si no responde (demo: ~5 min). */
export function mensajeRecordatorioParada({ direccion = null } = {}) {
  return (
    `Hola, ¿seguís *parado*?` +
    (direccion ? `\n📍 ${direccion}` : "") +
    `\n\n` +
    `Necesito que me digas *por qué* (tránsito, entrega, pinchazo, control…).\n` +
    `Si no hay respuesta en unos minutos, cierro la incidencia.`
  );
}

export function mensajeCierreSinRespuesta(row) {
  const codigo = codigoVisible(row);
  return (
    `Cierro la incidencia` +
    (codigo && codigo !== "—" ? ` *${codigo}*` : "") +
    ` por *falta de respuesta*.\n` +
    `Si algo sigue pasando, escribime *incidencia* y la abrimos de nuevo.`
  );
}
