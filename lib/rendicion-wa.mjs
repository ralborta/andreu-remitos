...
export function mensajeConfirmacionGasto(gasto, interpretacion) {
  ...
  const ctaLine = interpretacion?.combustible_cta_cte
    ? `\nImporte *${moneyAR(0)}* (cuenta corriente empresa).`
    : "";
  if (interpretacion?.mensaje && !interpretacion?.combustible_cta_cte) {
    return (
...
export function mensajeDuplicadoGasto(gastoExistente) {
  ...
  return (
    ...
    `No lo duplico. Si es otro, mandá bien la foto o avisá a mesa.`
  );
}

export function mensajeViajeAsignado(nroViaje) {
  return (
    `Ok ✅ Viaje Delfos *${nroViaje}* activo para rendición.\n\n` +
